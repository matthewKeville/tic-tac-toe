...
let granularity = 7; //determine resolution of individual spots
  ...
  if(key.keyCode === 13) {
    ...
    */
    ...
    if ((firstHit) && firstHit.distance != 0) {
      /*
      let sgeo = new THREE.SphereGeometry(30,200,200);
      sgeo.translate(firstHit.point.x,firstHit.point.y,firstHit.point.z);
      let smat = new THREE.MeshBasicMaterial({color: 0xff00f0});
      let sphere = new THREE.Mesh(sgeo,smat);
      scene.add(sphere);
      */
      //firstHit.object.geometry.setAttribute({color : 0xffff00 });
      firstHit.object.material.color = new THREE.Color( 0xff00ff * Math.random());
    }
  }
}
